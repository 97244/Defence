...
{
	idleMonsterArry = Array::create();// 用来保存空闲的怪物
	idleMonsterArry->retain();
	this->runMonsterArray = Array::create(); // 用来保存正在奔跑中的怪物
...
MonsterSprite* MonsterSystem::productMonster(int type) {
	if (type == 1) {
		MonsterSprite* sp = MonsterSprite::createWithMonsterRul(dutu.fileName, Director::getInstance()->getTextureCache()->getTextureForKey(dutu.picName), dutu.monsName, dutu.runCount, dutu.actCount, dutu.detCount);
		...
		sp->setMonState(1);
		sp->setAttackRange(RectFromString(dutu.attackRangeRec));
...
void MonsterSystem::recoverMonster(MonsterSprite* monsterSprite) {
	...
	if (monsterSprite->getMonType() == 1) {
		...
		monsterSprite->setPosition(Vec2::ZERO);
...
void MonsterSystem::monsterFormation(Array* monsterArrays) {
	if (monsterArrays) {
...
void  MonsterSystem::monsterFormationToOne(MonsterSprite* monsterSprite) {
	float ran = CCRANDOM_0_1();
	Size size = this->defenderGameLayer->getContentSize();
...
void MonsterSystem::addGold(int gold) {
	...
	UserDefault::getInstance()->setIntegerForKey("goldNum", goldnum);
	gameTipsSprite->setgoldNum(goldnum);


	int killtemp = UserDefault::getInstance()->getIntegerForKey("killtemp", 1);
	UserDefault::getInstance()->setIntegerForKey("killtemp", killtemp + 1);
