...
public:
	...
	CC_SYNTHESIZE(Rect, attackRange, AttackRange);// 接受攻击的范围
	...
	Rect converNowRect();// 这个方法是把最初设计的攻击范围 转化到当前 界面的坐标系中的矩形
protected:
	Vector<SpriteFrame*> runArray;//奔跑动画序列帧
	Vector<SpriteFrame*> deathArray;//死亡动画序列帧
	Vector<SpriteFrame*> attackArray;//攻击动画序列帧  
	ProgressTimer* bloodBwlid;// 这个是血条
	virtual void deathAnimationCallBack(Node* pSed);// 死亡动画回调函数
	virtual void attackAnimationCallBack(Node* pSed);// 攻击动画回调函数
	virtual void runAnimationCallBack(Node* pSed);//奔跑动画回调函数
