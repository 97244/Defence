...
bool WeaponSprite::initWithTexture(Texture2D *pTexture) {
	bool bRet = false;
	do
	{
		CC_BREAK_IF(!Sprite::initWithTexture(pTexture));
		// 初始话跑动的子弹
		this->pRunBulletsPool = Array::create();
		this->pRunBulletsPool->retain();
		this->magicArray = Array::create();
		magicArray->retain();
		//this->speed=10;
		this->hudu = 0;
		this->touchMg = false;
...
void WeaponSprite::initIdleBulletSpool(Layer* player) {
	// 创建 50个 子弹的纹理图片 这样做的目的是为了 加速游戏的 运行速度  
	auto texture = Director::getInstance()->getTextureCache()->getTextureForKey("game/wq0.png");
	batchNode = SpriteBatchNode::createWithTexture(texture, 50);
	batchNode->setPosition(Vec2::ZERO);
	player->addChild(batchNode, 2);
	// 初始化子弹池
	pIdleBulletsPool = Array::create();
	pIdleBulletsPool->retain();

	this->loadedArray = Array::create();
	loadedArray->retain();
	this->setZOrder(1);
	for (int i = 0; i < 50; i++) {
		// 创建子弹
		...
		pbullet->setHurt(UserDefault::getInstance()->getIntegerForKey("denfehurt", 10));
		// 把子弹放到子弹池中
...
WeaponSprite* WeaponSprite::createWithTexture(Texture2D *pTexture)
{
...
void WeaponSprite::loadedBullet(int cout) {
	if (this->loadedArray->count() != cout) {
		Array* tem = this->getIdleBullet(cout);
...
Array* WeaponSprite::getIdleBullet(int cout) {
	Array* ptemar = CCArray::create();
...
void WeaponSprite::initIdleMagicSpool(cocos2d::CCLayer* pLayer) {
	...
	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("specia/thumbnails.plist", Director::getInstance()->getTextureCache()->getTextureForKey("specia/thumbnails.png"));
	for (int i = 0; i < cout; i++) {
		...
		if (i == 2) {
			...
			if (magicMatrixSprite->getActivation()) {
				auto tem = CCSprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("diyus.png"));
			...
			magicMatrixSprite->setAnchorPo(ccp(0.5, 0.4));
			if (magicMatrixSprite->getActivation()) {
				auto tem = Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("longs.png"));
				tem->setPosition(ccp(magicMatrixSprite->getContentSize().width / 2, magicMatrixSprite->getContentSize().height / 2));
				magicMatrixSprite->addChild(tem);
			}
			...
			if (magicMatrixSprite->getActivation()) {
				auto tem = CCSprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("lights.png"));
				tem->setPosition(ccp(magicMatrixSprite->getContentSize().width / 2, magicMatrixSprite->getContentSize().height / 2));
				magicMatrixSprite->addChild(tem);
			}
...
MagicMatrixSprite* WeaponSprite::touchMagic(Point point) {
